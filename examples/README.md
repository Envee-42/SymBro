...
Four worked examples, in increasing order of how much infrastructure they need. Each is a folder with its own `README.md` — start with `01_full_walkthrough` regardless of which of the others you actually need, since it's the one that establishes the real, working baseline the others build on.
...
| [`04_analysis_notebook`](04_analysis_notebook/) | A local Jupyter notebook for scoring/exploring a completed run's `predict.pkl` — self-consistency RMSD vs. pLDDT plots, a per-assembly breakdown, an adjustable shortlist, structure preview | `symbro` itself installed (`pandas`/`matplotlib`, already base deps) and a completed run with at least one validated candidate in `.symbro/predict.pkl`. No GPU needed — it only reads checkpoints, doesn't run any stage. |
...
`04`'s notebook logic (the checkpoint joins, plots, shortlist/export) was executed for real with `jupyter nbconvert --execute` against synthetic-but-schema-correct `.symbro/*.pkl` fixtures and its outputs inspected — but, same GPU caveat as `01`/`03`, not against a real RFdiffusion/ProteinMPNN/predict run's actual output. See that folder's README for the exact verification/limitation details.
