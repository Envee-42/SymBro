...
### Structure-prediction backend status (as validated on our HPC cluster, Aug 2026)

The table above covers what each backend supports in principle; this covers what we've actually confirmed end-to-end on our own SLURM cluster, since "installs and runs" and "runs correctly at the performance you'd expect" aren't always the same thing in practice. None of the caveats below are symbro bugs — they're environment-specific (this cluster's driver/CUDA state), and are recorded here so the next person on this cluster doesn't have to re-diagnose them.

- **AlphaFold2/ColabFold (`af2`):** working end-to-end — a real SLURM job completed successfully via `symbro predict --predictor af2`. One caveat: the job log shows `WARNING: no GPU detected, will be using CPU` even though `colabfold_env` has CUDA-enabled `jax` installed — it hit the same driver/CUDA mismatch as AF3 (see below) and silently fell back to CPU rather than erroring. Results are correct, just slow; fine for a smoke test, not for production-sized batches until the underlying CUDA mismatch is resolved.
- **Boltz (`boltz`):** root cause found for an earlier failure — a corrupted local model cache (`~/.boltz/mols/`, missing ~12,000 files from an interrupted download/extraction), unrelated to symbro's own code. Re-extracting the cache is the fix; not yet reconfirmed with a clean end-to-end run (the retest job was killed mid-run by an unrelated session interruption, not a new failure).
- **AlphaFold3 (`af3`):** blocked on this specific cluster's hardware, not on symbro or AF3's config. The GPU node's NVIDIA driver is `525.60.13`, which per NVIDIA's own CUDA Toolkit / minimum-driver table is the exact minimum for CUDA 12.0 GA — one full release behind CUDA 12.1's own floor (`>=530.30.02`), let alone the CUDA 12.6 that AF3's pinned `jax[cuda12]` build actually wants (confirmed: the resolved `nvidia-cuda-runtime-cu12` was `12.6.77`). Because jax's pip-installable `cuda12` extra itself floors at CUDA 12.1 packages, pinning *older* `nvidia-*-cu12` wheels down to something this driver satisfies isn't viable — 12.1 is already above what `525.60.13` supports, so there's no pip-only pin that lands in range. The one real (untested) path forward without a driver upgrade is NVIDIA's [CUDA Forward Compatibility](https://docs.nvidia.com/deploy/cuda-compatibility/forward-compatibility.html) package (`cuda-compat-12-6`) — built for exactly this "data-center GPU, driver behind the toolkit" situation, though it typically needs cluster-admin installation rather than a user-level `pip`/`conda` install. Worth raising with cluster support before requesting a full driver upgrade, which is the more disruptive ask. Separately, worth double-checking however this was originally installed: AF3's own `jaxlib==0.4.34+cuda12` wheel was later pulled from PyPI upstream (see [google-deepmind/alphafold3#423](https://github.com/google-deepmind/alphafold3/issues/423)), so a *fresh* reinstall of this exact pin may not even resolve a GPU-capable wheel anymore, independent of the driver question.
